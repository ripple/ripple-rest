...
+ `secret` Account secret. Required for sending account_set transasction to change settings.
+ `limit` Trust limit. Either a string representation of trustline limit, `object` containing `value`, `currency`, `counterparty` or a `string` form `value/currency/counterparty`.
