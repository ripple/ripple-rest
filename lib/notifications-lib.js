...
function getNotification(remote, dbinterface, opts, callback) {
  ...
  if (validator.validate(identifier, 'Hash256').length > 0 && validator.validate(identifier, 'ResourceId').length > 0) {
  ...
  var steps = [
    ...
    function(async_callback) {
      ...
      async.concat([{descending: true}, {descending: false}], function(opts, concat_callback){
        ...
        getPossibleNextTransactionsFromRippled(remote, notification_details, opts, concat_callback);
    ...
    function(all_possible_transactions, async_callback) {
      ...
      var base_transaction_index = _.findIndex(possibilities, function(possibility){
        if (notification_details.tx.hash && possibility.hash === notification_details.tx.hash) {
          return true;
        }

        if (notification_details.tx.client_resource_id && possibility.client_resource_id === notification_details.tx.client_resource_id) {
          return true;
        }
 ...
 */
function getPossibleNextTransactionsFromRippled(remote, notification_details, opts, callback) {

  var steps = [

    function(async_callback) {
      accounttxlib.countAccountTransactionsInLedger(remote, notification_details.account, notification_details.tx.ledger_index, async_callback);
    },
    ...
    function(num_transactions_in_ledger, async_callback) {
      ...
      if (opts.descending) {
        ...
        params.ledger_index_max = notification_details.tx.ledger_index;
        params.ledger_index_min = -1;
      } else {
        params.descending = false;
        params.ledger_index_max = -1;
        params.ledger_index_min = notification_details.tx.ledger_index;
      }
