...
function getTransaction(remote, dbinterface, opts, callback) {
  ...
  var steps = [
    ...
    function(connected, async_callback) {
      dbinterface.getTransaction(opts, function(error, transaction){
        if (!transaction){
    ...
    function(transaction, async_callback) {
      ...
      if (opts.account) {
        ...
        if (!account_regex.test(transaction_string)) {
          return async_callback(new Error('Transaction unrelated to account. The transaction specified did not affect the given account'));
        }
      }
