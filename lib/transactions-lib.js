...
function getAccountTransactions(remote, dbinterface, opts, callback, previous_transactions) {
  ...
  async.waterfall(steps, function(err, transactions){
    ...
    console.log('min: ' + opts.min + ' transactions.length: ' + transactions.length + ' opts.marker: ' + JSON.stringify(opts.marker));
...
function getAccountTx(remote, opts, callback) {
  remote.requestAccountTx({
    ...
    marker: opts.marker
  }, function(err, account_tx_results){
    if (err) {
      callback(err);
      return;
    }

    console.log(account_tx_results);
