...
function getPayment(opts, callback) {
  ...
  var remote = opts.remote, 
    ...
    ledger_index = opts.ledger_index || opts.ledger;
  ...
  if (!address && !ledger_index) {
    callback(new Error('Must provide either an address or a ledger_index, as well as the tx_hash, to look up a payment'));
    return;
  }
...
function validateNewPayment(payment) {
  ...
  if (payment.paths) {
    try {
      ...
      throw(new TypeError('Invalid parameter: paths. Must be a valid JSON string'));      
    }
  }
