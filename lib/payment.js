...
function getPayment(opts, callback) {
  ...
  if (!address) {
    callback(new Error('Invalid parameter: address. getPayment() must be called with opts including a ripple address'));
    return;
  }
...
function paymentToTx(payment, opts) {
  ...
  if (payment.src_amount.currency !== 'XRP' && payment.src_amount.issuer === '') {
    payment.src_amount.issuer = payment.src_address;
  }
  if (payment.dst_amount.currency !== 'XRP' && payment.dst_amount.issuer === '') {
  ...
  if (payment.src_amount && 
    ...
    !(equal(payment.src_amount, payment.dst_amount))) {
