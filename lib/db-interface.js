...
function getOutgoingTransaction(outgoing_transactions, identifiers, callback) {
  outgoing_transactions.find({
    ...
    if (res && res.values) {
      ...
      var transaction = res.values.tx_json;
      transaction.meta = transaction.meta || {};
      transaction.meta.TransactionResult = res.values.result;
      transaction.ledger_index = res.values.ledger;
      transaction.hash = res.values.hash;
