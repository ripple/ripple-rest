...
DI.saveTransaction = function(transaction, callback) {
  ...
  .caught(function(err) {
    logger.info('[DB] Database Error. Cannot save transaction to database: '
                + err);
    (callback || new Function)(err);
  });
...
DI.moveValidatedTransactions = function() {
  ...
  .then(function(res) {
    if (res) {
      logger.info('[DB] Moved transactions to permanent table: ' + res);
    }
  })
  .caught(function(err) {
    logger.info('[DB] Database Error. Error moving validated transactions to '
                + 'permanent table: ' + err);
  });
