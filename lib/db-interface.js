...
function DatabaseInterface(options) {
  ...
  this.db = require('../db/db-connect')(options);
  ...
  this.db.sync().complete(handleDbSync);
};
    ...
    if (db_entry.values && db_entry.values.finalized) {
      self.moveValidatedTransactionsToPermanentTable(function(err, num_moved) {
        if (config.get('NODE_ENV') !== 'test') {
          if (err) {
            logger.info('Database Error. Error moving validated transactions from outgoing_transactions to client_resource_id_records table. ' + err);
            return;
          }

          logger.info(num_moved + ' transactions moved from outgoing_transactions table to client_resource_id_records table.');
        }
      });
    }
    callback(null, null);
