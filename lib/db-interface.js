...
DI.init = function(callback) {
  ...
  if (this.initialized) {
    logger.info('[DB] Database Warning. Re-initializing');
  }
  ...
  .then(function(res) {
    ...
    logger.info('[DB] Database initialized: ' + self.filePath);
  ...
  .caught(function(err) {
    logger.info('[DB] Database Error. Failed to initialize database:' + err);
...
DI.saveTransaction = function(transaction, callback) {
  ...
  var txData = {
    ...
    // Validated properteis
    rippled_result: transaction.result.engine_result,
    rippled_result_message: transaction.result.engine_result_message
  };

  txData.hash = transaction.result.transaction_hash !== void(0)
  ? transaction.result.transaction_hash
  : transaction.submittedIDs[0];

  txData.ledger = transaction.result.ledger_index !== void(0)
  ? transaction.result.ledger_index
  ...
  .caught(function(err) {
    logger.info('[DB] Database Error. Cannot save transaction to database: ' + err);
    (callback || new Function)(err);
  });
...
DI.moveValidatedTransactions = function() {
  ...
  .map(function(validatedEntry) {
    ...
    .whereNotExists(resourceIDEntry)
    .insert(resourceIDEntry);
  })
  .then(function(res) {
    if (res.length) {
      return self.db('outgoing_transactions')
  ...
  .then(function(res) {
    if (res) {
      logger.info('[DB] Moved transactions from outgoing_transactions table to '
                  + 'client_resource_ids: ' + res);
    }
  })
  .caught(function(err) {
    logger.info('[DB] Database Error. Error moving validated transactions from '
                + 'outgoing_transactions to client_resource_ids table: '
                + err);
  });
