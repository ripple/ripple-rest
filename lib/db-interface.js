      ...
      saveTransaction(outgoing_transactions, transaction_data, function(err){
        ...
        if (transaction_data.state === 'validated') {
          moveValidatedTransactionsToPermanentTable(outgoing_transactions, client_resource_id_records, function(err, num_moved){
            if (err) {
              console.log('Database Error. Error moving validated transactions from outgoing_transactionss to client_resource_id_recordss table. ' + err);
              return;
            }

            if (num_moved > 0) {
              console.log(num_moved + ' transactions moved from outgoing_transactionss to client_resource_id_recordss table.');
            }
          });
        }
      });
...
/**
 *  Once outgoing_transactionss have been validated they can be moved to the client_resource_id_recordss table
 *  and deleted from the outgoing_transactionss table, because they have been written into the Ripple Ledger.
  ...
  outgoing_transactions.findAll({
    ...
    async.reduce(results, 0, function(num_moved, outgoing_transactions_entry, async_callback){
      ...
      client_resource_id_records.findOrCreate({
        ...
        // it may be deleted from the outgoing_transactionss table
        // if (entry.created) {
...
/**
 *  Lookup a transaction in both the outgoing_transactionss and client_resource_id_recordss tables
