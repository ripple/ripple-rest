...
module.exports = function(opts) {
  ...
  return {
    ...
    getOutgoingTransaction: function(opts, callback) {
      getOutgoingTransaction(outgoing_transactions, opts, callback);
    },
...
function getFailedTransactions(outgoing_transactions, opts, callback) {
  var query = {
    where: {
      state: 'failed',
      finalized: true
...
function getOutgoingTransaction(outgoing_transactions, opts, callback) {
  var query = {
    where: {
      source_account: opts.account || opts.source_account
    }
  };
  ...
  outgoing_transactions.find(query).complete(function(err, res){
    if (err) {
      callback(err);
      return;
    }
    ...
    if (res && res.values) {
      callback(null, outgoingTransactionEntryToTx(res));
    } else {
      callback(null, null);
    }
  });
