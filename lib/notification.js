  ...
  remoteConnect.ensureConnected(remote, function(err, connected){
    ...
    getNextTx(remote, {
      ...
      if (next_tx) {
        ...
        // Attach source_transaction_id to outgoing transactions
  ...
  OutgoingTx
    ...
    .complete(function(err, tx){
      ...
      if (tx) {
        ...
        notification.source_transaction_id = tx.source_transaction_id;
        ...
        OutgoingTx
          .find({ where: {
            source_transaction_id: notification.hash
          }})
          .complete(function(err, tx){
            if (err) {
              callback(err);
    ...
    .complete(function(err, reported_txs){
      ...
      OutgoingTx
        .update({
          ...
          source_address: notification.address,
          source_transaction_id: notification.source_transaction_id
        })
      ...
      if (reported_txs && reported_txs.length > 0) {
        ...
        async.eachSeries(reported_txs, function(reported_tx, async_callback){
          if (reported_tx.values.source_transaction_id !== notification.source_transaction_id) {
...
function checkDbForUnreportedNotifications(OutgoingTx, notification, callback) {
  ...
  deleteAlreadyReportedNotifications(OutgoingTx, notification, function(err, deleted){
    if (err) {
      callback(err);
      return;
    }
    ...
    getFailedNotification(OutgoingTx, notification.address, function(err, failed_tx){
      ...
      if (failed_tx) {
        ...
        notification.source_transaction_id = failed_tx.source_transaction_id;
...
function deleteAlreadyReportedNotifications(OutgoingTx, notification, callback) {
  ...
  OutgoingTx
    ...
    .complete(function(err, txs){
      ...
      async.eachSeries(txs, function(tx, async_callback){
        if (tx.values && tx.values.hash !== notification.hash) {
          tx.destroy().complete(async_callback);
        } else {
          async_callback();
        }
      }, function(err){
      ...
      if (failed_tx) {
        OutgoingTx
          .update({
            ...
            source_transaction_id: failed_json.source_transaction_id,
