...
function parseNotificationFromTx(tx, opts, callback){
  ...
  var account = opts.account, 
    ...
    client_resource_id = opts.client_resource_id || tx.client_resource_id,
  ...
  if (tx.TransactionType) {
    ...
    if (notification.type === 'payment') {
      notification.transaction_url = '/v1/accounts/' + notification.account + '/payments/' + (tx.from_local_db ? notification.client_resource_id : notification.hash);
    } else {
      // TODO add support for lookup by client_resource_id for tx endpoint
      notification.transaction_url = '/v1/tx/' + notification.hash;
    }
