    ...
    direction: (opts.account ? 
      (opts.account === tx.Account ? 
        ...
        (opts.account === tx.Destination ? 
          ...
          'passthrough')) :
      ''),
    state: tx.state || (tx.meta.TransactionResult === 'tesSUCCESS' ? 'confirmed' : 'failed'),
...
function paymentToRippleLibTransaction(payment, callback) {
  ...
  paymentIsValid(payment, function(err, valid){
    ...
    var transaction = new ripple.Transaction(),
      transaction_data = {
        ...
        amount: (payment.destination_amount.currency === 'XRP' ? 
          utils.xrpToDrops(payment.destination_amount.value) :
    ...
    if (payment.source_tag) {
      transaction.sourceTag(payment.source_tag);
    }
    if (payment.destination_tag) {
      transaction.destinationTag(payment.destination_tag);
    }
...
module.exports.paymentToRippleLibTransaction = paymentToRippleLibTransaction;
