...
module.exports.getTx = function(remote, txHash, callback) {
  ...
  ensureRemoteConnected(remote, function(err, connected){
    ...
    remote.requestTx(txHash, function(err, tx){
      ...
      remote.requestLedger(tx.inLedger, function(err, ledgerHeader){
        if (err) {
          callback(err);
          return;
        }

        tx.close_time_unix = ripple.utils.toTimestamp(ledgerHeader.close_time);
  ...
  ensureRemoteConnected(remote, function(err, connected){
    ...
    if (tx.constructor.name === 'Transaction') {
      ...
      Object.keys(txJson).forEach(function(key){
        if (typeof txJson[key] === 'object' && txJson[key].currency === 'XRP' && !txJson[key].issuer) {
          txJson[key] = '' + txJson[key].value + 'XRP';
        }
      });
...
function txToNotification(notificationAddress, txEntry) {
  ...
  var notification = {
    ...
    txHash: tx.hash,
    txTimestamp: tx.close_time_unix || ripple.utils.toTimestamp(tx.date),
    submissionToken: ''
  };
