  ...
  function changeAccountSettings(connected, callback) {
    ...
    for (var flag in FlagSet) {
      ...
      if (opts[flag]) {
        ...
        //unset
        transaction.setFlags(FlagSet[flag].unset);
      }
    }
    ...
    for (var field in AccountRootFields) {
      ...
      if (AccountRootFields[field] === 'hex') {
        transaction.transaction_json[field] = new Buffer(opts[field]).toString('hex');
      } else {
        transaction.transaction_json[field] = opts[field];
      }
    }
